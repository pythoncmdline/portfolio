...
function adjustZoomAndCenterHorizontally() {
  const baseWidth = 1920; // Base width for 100% zoom
  const currentWidth = window.innerWidth;
  const scale = currentWidth / baseWidth; // Calculate the scale factor

  // Calculate horizontal offset to center the content
  const contentWidth = document.documentElement.offsetWidth * scale;
  const offsetX = (currentWidth - contentWidth) / 2; // Center horizontally

  // Apply transformations for scaling and horizontal centering
  document.documentElement.style.transform = `scale(${scale}) translateX(${offsetX}px)`;
  document.documentElement.style.transformOrigin = 'top left'; // Ensures correct anchor point for scaling
}

// Apply the function on window load and resize
